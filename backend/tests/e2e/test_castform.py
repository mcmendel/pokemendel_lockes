...
TEST_LOCKE = "CastformLocke"


def test_base_gen3(client_fixture):
    run_id = _create_run(client_fixture, GEN3_GAME_NAME)
    castform_id = _verify_run_created(
        client_fixture=client_fixture,
        run_id=run_id,
    )

   # nickname starter
    next_actions = get_next_actions(client_fixture, run_id, castform_id)
    assert next_actions == ["Nickname Pokemon"]
    nickname_options = get_action_options(client_fixture, run_id, castform_id, "Nickname Pokemon")
    assert nickname_options == {
        "input_options": [],
        "input_type": "Free text"
    }
    execute_action(client_fixture, run_id, castform_id, "Nickname Pokemon", "mystarter")

    # verify mandatory actions
    ### 1. gender
    next_actions = get_next_actions(client_fixture, run_id, castform_id)
    assert next_actions == ['Gender', 'Nature', 'Ability']
    action_options = get_action_options(client_fixture, run_id, castform_id, "Gender")
    assert action_options == {
        "input_options": ["Male", "Female"],
        "input_type": "One of"
    }
    execute_action(client_fixture, run_id, castform_id, "Gender", "Male")
    run_response = get_run(client_fixture, run_id)
    assert run_response["pokemons"][castform_id]["metadata"]["gender"] == "Male"

    ### 2. nature
    next_actions = get_next_actions(client_fixture, run_id, castform_id)
    assert next_actions == ['Nature', 'Ability']
    action_options = get_action_options(client_fixture, run_id, castform_id, "Nature")
    ...
    execute_action(client_fixture, run_id, castform_id, "Nature", "Docile")
    run_response = get_run(client_fixture, run_id)
    assert run_response["pokemons"][castform_id]["nature"] == "Docile"

    ### 3. ability
    next_actions = get_next_actions(client_fixture, run_id, castform_id)
    assert next_actions == ['Ability']
    action_options = get_action_options(client_fixture, run_id, castform_id, "Ability")
    assert action_options == {
        "input_options": [Abilities.FORECAST],
        "input_type": "One of"
    }
    execute_action(client_fixture, run_id, castform_id, "Ability", Abilities.FORECAST)
    run_response = get_run(client_fixture, run_id)
    assert run_response["pokemons"][castform_id]["metadata"]["ability"] == Abilities.FORECAST
...
def _verify_run_created(client_fixture, run_id):
    run_response = get_run(client_fixture, run_id)
    assert len(run_response['run']['box']) == 4  # castform and its 3 forms
    castform_id = run_response['run']['starter']
    assert run_response['pokemons'][castform_id]['name'] == PokemonGen3.CASTFORM

    all_pokemons = {
        run_response['pokemons'][pokemon_id]['name']
        for pokemon_id in run_response['pokemons'].keys()
    }
    assert all_pokemons == {PokemonGen3.CASTFORM, PokemonGen3.CASTFORM_SUNNY, PokemonGen3.CASTFORM_RAINY, PokemonGen3.CASTFORM_SNOWY}
    return castform_id
